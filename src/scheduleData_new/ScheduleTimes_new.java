 ...
 */
public class ScheduleTimes_new implements TableModel {

    private static final int DAYS = 6, COLUMNS = 3; // 6 = Mo-Sa 

    private static final ScheduleDay_new[] SCHEDULEDAY_LIST = new ScheduleDay_new[DAYS];  // fixe interne Liste aller Unterrichtstage für TableModel
    ...
    private ArrayList<ScheduleDay_new> scheduleDayList; // dynamische Liste und neues Mapping mit den ausgewählten Unterrichtstagen zur Weiterverwendung nach aussen

    public ScheduleTimes_new() {

        for (int i = 0; i < DAYS; i++) {
            SCHEDULEDAY_LIST[i] = new ScheduleDay_new();  // ScheduleDays werden setValueAt() definiert
        }
        scheduleDayList = new ArrayList<>();
    }

    /* Getter, Setter */
    public ScheduleDay_new getScheduleDay(int i) {
